...
func ExcluirUsuario(w http.ResponseWriter, r *http.Request) {
	...
	if usuarioID != usuarioIDNoToken {
		respostas.Erro(w, http.StatusForbidden, errors.New("não é possível excluir um usuário diferente do seu"))
		return
	}
...
func SeguirUsuario(w http.ResponseWriter, r *http.Request) {
	...
	if seguidorID == usuarioID {
		respostas.Erro(w, http.StatusForbidden, errors.New("não é possível um usuário seguir a si mesmo"))
		return
	}
...
// BuscarSeguindo busca usuários que seguem um determinado usuário
...
func AtualizarSenha(w http.ResponseWriter, r *http.Request) {
	...
	if usuarioIDNoToken != usuarioID {
		respostas.Erro(w, http.StatusForbidden, errors.New("não é possível atualizar a senha de um usuário diferente do seu"))
		return
	}
	...
	repositorio := repositorios.NovoRepositorioDeUsuarios(db)
	senhaGravadaNoBanco, erro := repositorio.BuscarSenha(usuarioID)
	if erro != nil {
		respostas.Erro(w, http.StatusInternalServerError, erro)
		return
	}

	if erro = seguranca.VerificarSenha(senhaGravadaNoBanco, senha.Atual); erro != nil {
		respostas.Erro(w, http.StatusUnauthorized, errors.New("a senha atual difere da nova"))
		return
	}
