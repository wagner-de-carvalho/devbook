...
func CriarPublicacao(w http.ResponseWriter, r *http.Request) {
	...
	publicacao.AutorID = int64(usuarioID)
...
// BuscarPublicacoes busca as publicações do 'feed' do usuário
...
func AtualizarPublicacao(w http.ResponseWriter, r *http.Request) {
	...
	repositorio := repositorios.NovoRepositorioDePublicacoes(db)
	publicacaoGravadaNoBanco, erro := repositorio.BuscarPorID(publicacaoID)
	if erro != nil {
		respostas.Erro(w, http.StatusInternalServerError, erro)
		return
	}

	if publicacaoGravadaNoBanco.AutorID != int64(usuarioID) {
		respostas.Erro(w, http.StatusForbidden, errors.New("não é ermitido atualizar publicação de outrém"))
		return
	}
...
// ExcluirPublicacao exclui os dados de uma publicação
func ExcluirPublicacao(w http.ResponseWriter, r *http.Request) {
	...
	if publicacaoGravadaNoBanco.AutorID != int64(usuarioID) {
		respostas.Erro(w, http.StatusForbidden, errors.New("não é ermitido excluir publicação de outrém"))
		return
	}
